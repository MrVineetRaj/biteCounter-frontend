...
const Home = () => {
  ...
  const fetchUserDiet = async () => {
    try {
      const response = await axios.get('https://bitecounter-backend.onrender.com/user/diet', {
        headers: {
  ...
  const addFood = async (foodTime, food) => {
    try {
      const response = await fetch(`https://bitecounter-backend.onrender.com/${foodTime.toLowerCase()}/added`, {
        method: 'POST',
        headers: {
  ...
  const handleSavedUse = async () => {
    try {
      const response = await fetch(`https://bitecounter-backend.onrender.com/user/diet/save`, {
        method: 'get',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${auth.token}` // Assuming you're using JWT token for authentication
    ...
    <center>
      <div className={`${style.container}`}>
        ...
        <div className={`${style.bmiDetails}container text-center`}>
          <div className="row">
            <div className="col">
              <span type="button" className="btn btn-primary">Your BMI : {BMI}</span>
            </div>
            <div className="col">
              {BMI > 18.4 && BMI <= 24.3 ? <span type="button" className="btn btn-success">You BMI is Perfect</span> : (BMI < 18.4 ? <span type="button" className="btn btn-warning">You are Underweight</span> : <span type="button" className="btn btn-warning">Your are overweight</span>)}
            </div>
          </div>
        </div>
        <div className={`${style.saveDiet} container text-center`}
          ...
          <div className="row">
            {
              diet.map((item, index) => {
                return <div className="col" key={index}>
                  <span key={item} type="button" className="btn btn-info"
                    style={{
                      fontWeight: "Bold",
                      color: "white",
                      margin: "10px 0"
                    ...
                    <div className="row">
                      <div className={`${style.colm} col`}>
                        ...
                        <button type="button" className={`${style.btnn} btn btn-success`} onClick={handleSavedUse}>
                          Use___<MdSaveAlt />
                        </button>
                      </div>
                      <div className={`${style.colm} col`}>

                        <button type="button" className={`${style.btnn} btn btn-warning`} onClick={handleSavedSave}>
                          Save__<CiSaveUp2 />
                        </button>
                      </div>
                    </div>
        ...
        <Diet dayDiet={userDiet} fetchUserDiet={fetchUserDiet} savedDiet={"onetime"} />
