...
export const TableCalorie = ({ foodTime, time, savedDiet, fetchUserDiet }) => {
  ...
  const deleteFood = async (food) => {
    try {
      const response = await fetch(`https://bitecounter-backend.onrender.com/${time}/food/${savedDiet}/delete`, {
        method: 'DELETE',
        headers: {
          ...
          {foodTime.map((food, index) => (
            <><tr key={index} >
              ...
              <td>{food.calories}</td>
                ...
                <td  colSpan={3}> 
                  <button type="button" className="btn btn-danger" onClick={() => deleteFood(food)} >
                    Delete <MdDelete />
                  </button>
                </td>
              </tr>
