...
function Login() {
  ...
  const onLogin = async (event) => {
    ...
    //Setting Auth Token 
    const response_auth = await fetch('https://bitecounter-backend.onrender.com/user/login', {
      method: 'POST',
      headers: {
    ...
    <>
      <center className={style.bg}>
        ...
        <form action="" className={style.form} onSubmit={onLogin}>
          ...
          { !isValidPassword &&  <> 
            <div class="alert alert-danger" role="alert">Your password or/and email is/are incorrect...
            </div>
          </>}
          <div  className="mb-3">
          ...
          <p>Dont have an account ? <span><Link to={"/sign-up"}>Sign Up</Link> </span></p>
        </form>
      </center>
    </>
  )
